...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--approach', type=int, default=2, help='which DEXOM approach to use')
    ...
    if args.approach == 2:
        ...
        imatsol, imatbin = dexom_python.read_solution(outpath+'imat_solution_%s.csv' % condition)
        rxnenum_sols = pd.read_csv(outpath + 'all_rxn_enum_solutions_%s.csv' % condition, index_col=0)
        rxnenum_sols.columns = imatsol.fluxes.index
        prevsol = imatsol
